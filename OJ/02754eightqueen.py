...
def eight_queen():
    inp = tuple(int(input()) for _ in range(int(input())))
    n = max(inp)
    cnt = 0
    for queens in tuple(permutations(range(8)))[2842:]:
        if all(abs(queens[i] - queens[j]) != i - j for i in range(1, 8) for j in range(i)):
            cnt += 1
            if cnt in inp:
                print(''.join(map(lambda x: str(int(x) + 1), queens)))
        if cnt == n:
            break
...
def make_list():
    ...
    for queens in tuple(permutations(range(8)))[2842:]:
        if all(abs(queens[i] - queens[j]) != i - j for i in range(1, 8) for j in range(i)):
            ans.append(''.join(map(lambda x: str(int(x) + 1), queens)))
    print(ans)


def trick_solution():
    lst = ['15863724', '16837425', '17468253', '17582463', '24683175', '25713864',
